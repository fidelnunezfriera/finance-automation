@echo off
REM ===========================================================================
REM  Recalcula merchant_norm en TODAS las filas que ya estan en la hoja, a
REM  partir de su merchant_raw. Hace falta despues de un arreglo en
REM  norm_merchant() (pipeline\convert_pytr_to_clean.py), porque ese arreglo
REM  solo afecta a exports nuevos -- las filas ya escritas se quedan con el
REM  merchant_norm viejo hasta este backfill.
REM
REM  Primero simula y ensena que cambiaria. Luego pregunta si aplicarlo. Si
REM  dices que no, no se escribe nada.
REM
REM  Como merchant_norm cambia, tambien puede cambiar que regla coincide, asi
REM  que de paso se recalculan category, subcategory, rule_id y
REM  rule_confidence. El resto de columnas no se tocan. No se conecta a
REM  Trade Republic.
REM ===========================================================================
...
.venv\Scripts\python.exe pipeline\backfill_merchant_norm.py --dry-run
...
echo Aplicando...
echo.
.venv\Scripts\python.exe pipeline\backfill_merchant_norm.py
